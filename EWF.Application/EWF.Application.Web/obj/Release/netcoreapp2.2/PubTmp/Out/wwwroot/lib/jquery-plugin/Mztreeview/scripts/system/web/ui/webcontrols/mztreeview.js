...
MzTreeView.addProperty("defaultUrl", "javascript:void(0);");
...
MzTreeView.prototype.render = function()
{
 ...
 // alert(str);
  ...
  return "<div class='mztreeview' id='MTV_root_"+ this.index +"' "+
    ...
    "oncontextmenu='Instance(\""+ this.index +"\").contextMenuHandle(event)' "+
    ">"+ str +"</div>";
};
...
//  // e.outerHTML="zhw";  (/\\/g,'/')
//  var len=e.outerHTML.indexOf("javascript:");
//  var len2=e.outerHTML.indexOf("?>");
//  var s1=e.outerHTML.substring(0,len);
//  var s2=e.outerHTML.substring(len2,e.outerHTML.length);
//   e.outerHTML=s1+"javascript:winOpen('"+this.currentNode.id+"','"+this.currentNode.text+"');"+s2;//
//  }
      ...
      if(B)
      {
        if(e.id.indexOf(this.index +"_expand_")==0){
          ...
          n.focus();}
            ...
            n.check(!n.checked); n.upCheck(); //LayerControl(n.id, n.text, n.checked);
          }
      }
      break;
    case "A":
...
MzTreeView.prototype.collapseAll = function(id)
{
 ...
 MzTreeView.prototype.collapseOAll = function(id)
{
    this.collapseAll('-1');
    var strPath='_'+id;
    var strNode;
    if(this.currentNode.parentNode.id!='-1') 
    {
       strNode=this.currentNode.parentNode;
       strPath='_'+strNode.id+strPath;
       if(strNode.parentNode.id!='-1')
       {
        strNode=strNode.parentNode;
        strPath='_'+strNode.id+strPath;
         if(strNode.parentNode.id!='-1')
           {
            strNode=strNode.parentNode;
  ...
  if(node.hasChild)
  {
   ...
   strPath=strPath+"_"+node.childNodes[0].id;
   }
   if(strPath.substring(0,1)=='_')
   {
    strPath="-1"+strPath;
   }
  this.focusNodeByPath(strPath);
  //var node=this.getNodeById(id);
 //  node.expand();
   
   //this.collapseAll('-1');
  /* var i, root=this.rootNode;
     if (root.hasChild){
 
    var a = [], c = root.childNodes;
     var id=this.currentNode.id;
     for(i=0;i<c.length;i++)
     {
       if(c[i].expanded&&c[i].id!=id)
        {
          if(c[i].hasChild)
          {
...
MzTreeNode.prototype.gets=function(id)
{
 //debugger;
...
MzTreeNode.prototype.render = function(last)
{
  ...
  s += "<div><table border='0' cellpadding='0' cellspacing='0'>"+
       "<tr title='"+ hint +"'><td>"; if (MzTreeNode.htmlChildPrefix&&!isRoot)
...
MzTreeNode.prototype.updateNodeLink=function()
{
 ...
 // if(!(url=s.getAttribute("url"))) url = $.getDefaultUrl();
  ...
  if(url.indexOf(')')>-1)
  { 
  //var tlen=url.indexOf(')');
  //url=url.substring (0,tlen+1)+";";
