...
{
    public class EstatService : IEstatService
    {
        private IEstatRepository repository;
        public EstatService(IEstatRepository _epository)
        {
            repository = _epository;
        }

        public IEnumerable<ST_ESTATEntity> GetDayData(string STCD, string sdate, string edate, int type, string addvcd, ref string datasrc)
        {
            ...
            var list_real = repository.GetDayData_ByDAYEV(STCD, stime, etime, type, addvcd);
        ...
        //表格转置-多日累计
        private IEnumerable<ST_ESTATEntity> ConvertTableDay(IEnumerable<ST_ESTATEntity> list)
        {
            var list_result = new List<ST_ESTATEntity>();
            ...
            foreach (var item in list)
            {
                ...
                SUMACCE += item.ACCE.SafeValue();
                ST_ESTATEntity row = new ST_ESTATEntity
                {
                    STCD = item.STCD,
                    STNM = item.STNM,
                    IDTM = item.IDTM,
                    STTDRCD = item.STTDRCD,
                    ACCE = item.ACCE.SafeValue().ToString("F1").ToDouble(),
                    SUMACCE = SUMACCE.ToString("F1").ToDouble(),
                };
                list_result.Add(row);
            }
            return list_result;
        ...
        //表格转置-旬月累计
        private IEnumerable<dynamic> ConvertTableMonth(IEnumerable<ST_ESTATEntity> list)
        {
            ...
            foreach (var item in list)
            {
                ...
                switch (IDT) {
                    case 1:
                        row_new.SXun = item.ACCE.SafeValue().ToString("F1").ToDouble();
                        break;
                    case 2:
                        row_new.ZXun = item.ACCE.SafeValue().ToString("F1").ToDouble();
                        break;
                    case 0:
                        row_new.XXun = item.ACCE.SafeValue().ToString("F1").ToDouble();
                        break;
                    case 4:
                        row_new.MonSum = item.ACCE.SafeValue().ToString("F1").ToDouble();
                        break;
                    //default:
        ...
        //表格转置-年累计
        private DataTable ConvertTableYear(IEnumerable<ST_ESTATEntity> list)
        {
            #region
            ...
            foreach (var row in list)
            {
                ...
                row_new[IDT + 2] = Convert.ToDouble(row.ACCE).QFormat(4);
            }
        ...
        /// <param name="year">对比年份</param>
        /// <returns>时段内旬月均值</returns>
        public IEnumerable<dynamic> GetData_MXunEV(string STCD, string type, string sdate, string edate, string sten, string eten, string year, ref string datasrc)
        {
            var stime = "";
            var etime = "";
            var sdate_history = "";
